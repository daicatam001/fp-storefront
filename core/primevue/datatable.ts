...
export default {
    ...
    loadingoverlay: {
        class: ['absolute', 'top-0 left-0', 'z-20', 'flex items-center justify-center', 'w-full h-full', 'bg-slate-100/40 dark:bg-slate-800/40', 'transition duration-200']
    },
    loadingicon: {
    ...
    thead: ({ context }) => ({
        class: [
            {
                ' dark:bg-slate-800 top-0 z-40 sticky': context.scrollable
            }
        ]
    }),
    ...
    footer: {
        class: ['font-bold', 'border-t-0 border-b border-x-0', 'p-4', ' dark:bg-slate-800', 'border-slate-200 dark:border-slate-700', 'text-slate-700 dark:text-white/80']
    },
    column: {
        headercell: ({ context, props }) => ({
            class: [
                ...
                // Color
                (props.sortable === '' || props.sortable) && context.sorted ? 'bg-primary-highlight text-primary-highlight-inverse' : ' text-slate-700 dark:text-white/80 dark:bg-slate-800',
                'border-slate-200 dark:border-slate-700 ',
                // States
                { 'hover:bg-slate-100 dark:hover:bg-slate-400/30': (props.sortable === '' || props.sortable) && !(context != null && context.sorted) },
        ...
        bodycell: ({ props, context, state, parent }) => ({
            class: [
                ...
                { 'bg-slate-0 dark:bg-slate-800': parent.instance.frozenRow || props.frozen || props.frozen === '' },
                // Spacing
        ...
        sorticon: ({ context }) => ({
            class: ['ml-2', context.sorted ? 'text-primary-highlight-inverse' : 'text-slate-700 dark:text-white/70']
        }),
        sortbadge: {
        ...
        filteroverlay: {
            class: ['absolute top-0 left-0', 'border-0 dark:border', 'rounded-md', 'shadow-md', 'min-w-[12.5rem]', 'bg-slate-0 dark:bg-slate-800', 'text-slate-800 dark:text-white/80', 'dark:border-slate-700']
        },
        ...
        filterrowitem: ({ context }) => ({
            class: [
                ...
                'py-3 px-5',
                // Color
                { 'bg-slate-0 dark:bg-slate-800 text-slate-700 dark:text-white/80': !(context != null && context.highlighted) },
                { 'bg-primary-highlight text-primary-highlight-inverse': context == null ? void 0 : context.highlighted },
                //States
                { 'hover:bg-slate-100 dark:hover:bg-slate-600/80': !(context != null && context.highlighted) },
                { 'hover:text-slate-700 hover:bg-slate-100 dark:hover:text-white dark:hover:bg-slate-600/80': !(context != null && context.highlighted) },
                'focus-visible:outline-none focus-visible:outline-offset-0 focus-visible:ring focus-visible:ring-inset focus-visible:ring-primary-400/50 dark:focus-visible:ring-primary-300/50',
                // Transitions
                'transition-shadow',
                'duration-200',
        ...
        filteroperator: {
            class: ['px-5 py-3', 'border-b border-solid', 'rounded-t-md', 'text-slate-700 dark:text-white/80', 'border-slate-200 dark:border-slate-800', 'bg-slate-0 dark:bg-slate-700']
        },
        filteroperatordropdown: {
            ...
            input: ({ props }) => ({
                class: [
                    ...
                    'bg-transparent',
                    'border-0',
                    { 'text-slate-800 dark:text-white/80': props.modelValue, 'text-slate-400 dark:text-slate-500': !props.modelValue },
                    'placeholder:text-slate-400 dark:placeholder:text-slate-500',
                    // Sizing and Spacing
                    'w-[1%]',
                    'p-3',
            ...
            item: ({ context }) => ({
                class: [
                    ...
                    { ' dark:bg-slate-600/60 text-slate-700 dark:text-white/80': context.focused && !context.selected },
                    ...
                    { 'hover:text-slate-700 hover:bg-slate-100 dark:hover:text-white dark:hover:bg-slate-600/80': context.focused && !context.selected },
                    // Transitions
                    'transition-shadow',
                    'duration-200',
                    // Misc
        ...
        filterconstraint: {
            class: ['py-3 px-5', 'border-b border-solid', 'border-slate-200 dark:border-slate-700']
        },
        filteraddrule: {
            class: 'py-3 px-5'
        },
        ...
        filtermenubutton: ({ context }) => ({
            class: [
                ...
                'dark:text-white/70 dark:hover:text-white/80 dark:bg-slate-800',
                // States
        ...
        rowtoggler: {
            class: ['relative', 'inline-flex items-center justify-center', 'text-left', 'm-0 p-0', 'w-8 h-8', 'border-0 rounded-full', 'text-slate-500 dark:text-white/70', 'bg-transparent', 'focus-visible:outline-none focus-visible:outline-offset-0', 'focus-visible:ring focus-visible:ring-primary-400/50 dark:focus-visible:ring-primary-300/50', 'transition duration-200', 'overflow-hidden', 'cursor-pointer select-none']
        },
        columnresizer: {
        ...
        roweditorinitbutton: {
            class: ['relative', 'inline-flex items-center justify-center', 'text-left', 'w-8 h-8', 'border-0 rounded-full', 'text-slate-700 dark:text-white/70', 'border-transparent', 'focus:outline-none focus:outline-offset-0 focus:ring focus:ring-primary-400/50 dark:focus:ring-primary-300/50', 'hover:text-slate-700 hover:bg-slate-300/20', 'transition duration-200', 'overflow-hidden', 'cursor-pointer select-none']
        },
        roweditorsavebutton: {
            class: ['relative', 'inline-flex items-center justify-center', 'text-left', 'w-8 h-8', 'border-0 rounded-full', 'text-slate-700 dark:text-white/70', 'border-transparent', 'focus:outline-none focus:outline-offset-0 focus:ring focus:ring-primary-400/50 dark:focus:ring-primary-300/50', 'hover:text-slate-700 hover:bg-slate-300/20', 'transition duration-200', 'overflow-hidden', 'cursor-pointer select-none']
        },
        ...
        rowRadiobutton: {
            ...
            box: ({ props }) => ({
                class: [
                    ...
                    // Colors
                    {
                        'text-slate-700 dark:text-white/80': !props.modelValue,
                        'bg-slate-0 dark:bg-slate-900': !props.modelValue,
                        'border-slate-300 dark:border-slate-700': !props.modelValue,
            ...
            input: {
                class: ['peer', 'w-full ', 'h-full', 'absolute', 'top-0 left-0', 'z-10', 'p-0', 'm-0', 'opacity-0', 'rounded-md', 'outline-none', 'border-2 border-slate-200 dark:border-slate-700', 'appearance-none', 'cursor-pointer']
            },
            icon: ({ props }) => ({
                class: [
                    'block',
        ...
        headercheckbox: {
            ...
            box: ({ props, context }) => ({
                class: [
                    ...
                    // Colors
                    {
                        'border-slate-200 bg-slate-0 dark:border-slate-700 dark:bg-slate-900': !context.checked,
            ...
            icon: {
                class: ['text-base leading-none', 'w-4', 'h-4', 'text-white dark:text-slate-900', 'transition-all', 'duration-200']
            }
        },
        rowCheckbox: {
            root: {
    ...
    bodyrow: ({ context, props }) => ({
        class: [
            ...
            { 'bg-slate-0 text-slate-600 dark:bg-slate-800': !context.selected },
            { 'font-bold bg-slate-0 dark:bg-slate-800 z-20': props.frozenRow },
            { 'odd:bg-slate-0 odd:text-slate-600 dark:odd:bg-slate-800 even: even:text-slate-600 dark:even:bg-slate-900/50': context.stripedRows },
            // State
    ...
    rowexpansion: {
        class: 'bg-slate-0 dark:bg-slate-800 text-slate-600 dark:text-white/80'
    },
    rowgroupheader: {
        class: ['sticky z-20', 'bg-slate-0 text-slate-600 dark:text-white/70', 'dark:bg-slate-800']
    },
    rowgroupfooter: {
        class: ['sticky z-20', 'bg-slate-0 text-slate-600 dark:text-white/70', 'dark:bg-slate-800']
    },
